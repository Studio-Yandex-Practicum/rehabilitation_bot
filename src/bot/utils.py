...
async def update_obscene_words_db_table() -> None:
    ...
    async with async_session() as session:
        with open('forbidden_words.json', 'r') as json_file:
...
async def get_all_obscene_words(session: AsyncSession) -> list[str]:
    """Получение списка всех слов ненормативной лексики"""
    words = await CRUDBase(ObsceneWordData).get_multi(session)
